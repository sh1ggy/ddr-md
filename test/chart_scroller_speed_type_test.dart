...
void main() {
  ...
  testWidgets('REAL SPEED ignores a transient spike: SMASH reads the full 600',
    ...
    // A SMASH-shaped chart: the playhead parked in a 160 head section, a long
    // sustained 160 main tempo, and a 0.8s 320 flash that never clears the 2s
    // sustain window. true_max stays 320, but the divisor is 160.
    await tester.pumpWidget(_host(ChartScroller(
      ...
      chartBpm: 160,
      minBpm: 80,
      maxBpm: 320, // true_max: preserved, but NOT the divisor
      bpms: [
        Bpm(st: 0, ed: 1, val: 160), // playhead sits here
        Bpm(st: 1, ed: 12, val: 160), // long sustained main tempo
        Bpm(st: 12, ed: 12.8, val: 320), // 0.8s transient spike
        Bpm(st: 12.8, ed: 22, val: 160),
        Bpm(st: 22, ed: 30, val: 80),
      ],
      stops: const [],
    )));
